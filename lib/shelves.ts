...
// 棚のリンク先だけが欲しい場面用（トップのシーンリンクなど）。表示名は copy.curated.titles から引く。
export function shelfLinks(ids: string[]): { id: string; href: string }[] {
  return ids
    .map((id) => getShelf(id))
