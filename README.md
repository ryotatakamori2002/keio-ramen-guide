...
- **文言（コピー）を直したい時は [`content/site-copy.ts`](content/site-copy.ts) を編集する。** サイト内の主要文言（サービス名・ナビ・Hero・CTA・セクション名・空状態・フォーム項目・管理画面・metadata / OGP など）はすべてここに集約している。各ページ/コンポーネントには直書きしない方針。
